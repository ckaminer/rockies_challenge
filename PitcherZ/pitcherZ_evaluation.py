...
for i in range(2, 85):
    ...
    strikeout_BB = z_data.cell(row=i, column=18).value
    hit_type = z_data.cell(row=i, column=20).value
    out_on_play = z_data.cell(row=i, column=21).value
    run_on_play = z_data.cell(row=i, column=22).value
    plate_appearance = z_data.cell(row=i, column=2).value
    previous_appearance = z_data.cell(row=i-1, column=2).value
